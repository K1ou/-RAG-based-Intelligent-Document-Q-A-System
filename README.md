...
本项目目标是提供一个可运行、可复现、可量化评估的端到端 Demo，便于后续工程化扩展。
...
当前阶段已通过文本化方式记录核心验证结果，无需依赖截图：

- 服务可用性：API 与 UI 可正常启动并访问（8000/7860）。
- 文档入库能力：支持 PDF/DOCX 上传、分块入库、向量持久化。
- 问答链路能力：支持 dense/hybrid/hybrid_rerank、查询改写、多轮会话记忆与引用溯源。
- 引用质量：已支持 strict_citation_hit_rate（source_file + source_location 严格匹配）指标。
- 评测结论：
	- 10 条样本：Ollama 在 `hybrid`、`hybrid_rerank` 的 answer_hit_rate 优于 Echo。
	- 60 条样本：Ollama 在三种模式下 answer_hit_rate 明显高于 Echo，且 out_of_scope 拒答质量更好；代价是平均延迟从毫秒级上升到秒级。
